...
class EzecomSupabaseUser extends BaseAuthUser {
  EzecomSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> ezecomSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = EzecomSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
