...
const String DENO_CREATE_PHONEPE_PAYMENT_SESSION_URL =
    'https://gticwwxjzuftuvilszmq.supabase.co/functions/v1/create-phonepe-payment-session-plan-purchase';
const String REACT_PAYMENT_PAGE_BASE_URL = 'https://ezecom.shop/payment/';

Future<bool> initiatePhonePePaymentPlanPurchase(
  String enquiryId,
  String userJwt,
) async {
  try {
    ...
    if (createSessionResponse.statusCode == 200) {
      ...
      if (responseBody['success'] == true &&
        ...
        final reactPaymentUrl =
            '$REACT_PAYMENT_PAGE_BASE_URL?sessionToken=$enquiryId'
            '&platform=$platform&uvid=${Uri.encodeComponent(encryptedUuid)}&planValidation=true';
        // 6️⃣ Launch URL
