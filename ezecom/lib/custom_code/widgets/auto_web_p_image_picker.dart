...
class AutoWebPImagePicker extends StatefulWidget {
  const AutoWebPImagePicker({
    Key? key,
    this.width,
    this.height,
    this.spacing = 10.0,
  ...
  @override
  State<AutoWebPImagePicker> createState() => _AutoWebPImagePickerState();
}

class _AutoWebPImagePickerState extends State<AutoWebPImagePicker> {
  ...
  Future<Uint8List?> _convertToWebP(Uint8List imageBytes) async {
    try {
      // Decode the image
      final ui.Codec codec = await ui.instantiateImageCodec(
        imageBytes,
        // Don't resize during decode to preserve quality
        targetWidth: null,
        targetHeight: null,
      );
      ...
      // Convert to PNG format with maximum quality
      // Note: Flutter's ui.Image.toByteData doesn't support WebP directly
      // PNG is lossless, so quality is preserved
      ...
      print('DEBUG: Error converting image: $e');
    }
    return null;
  }

  // Alternative WebP conversion using the 'image' package
  // You need to add 'image: ^4.1.7' to your pubspec.yaml dependencies
  Future<Uint8List?> _convertToWebPWithImagePackage(
      Uint8List imageBytes) async {
    try {
      // Uncomment and use this if you add the 'image' package
      /*
      ...
      if (originalImage != null) {
        // Convert to WebP with high quality (0-100, where 100 is best quality)
        Uint8List webpBytes = img.encodeWebP(originalImage, quality: 90);
        return webpBytes;
      }
      */

      // For now, return the original bytes without additional compression
      return imageBytes;
    } catch (e) {
      print('DEBUG: Error converting to WebP: $e');
      return null;
    }
  }
    ...
    try {
      ...
      if (source != null) {
        final XFile? image = await _picker.pickImage(
          source: source,
          maxWidth: 2048, // Increased from 1024
          maxHeight: 2048, // Increased from 1024
          imageQuality: 95, // Increased from 85
        );
        ...
        if (image != null) {
          ...
          if (image.name.toLowerCase().endsWith('.webp')) {
            webpBytes = bytes;
            base64String = 'data:image/webp;base64,${base64Encode(bytes)}';
          } else {
            // Convert other formats to WebP
            webpBytes = await _convertToWebPWithImagePackage(bytes);
            if (webpBytes != null) {
              base64String =
                  'data:image/webp;base64,${base64Encode(webpBytes)}';
              _showSuccessSnackBar(
                  'Image converted to WebP format successfully!');
            } else {
              // Fallback: use original image with appropriate MIME type
              ...
              _showWarningSnackBar(
                  'Image uploaded as ${mimeType.split('/')[1].toUpperCase()}. Consider using WebP for better compression.');
            }
          }
          ...
          try {
            ...
            // print(
            //     'DEBUG: Image $index loaded and FlutterFlow state updated successfully');
          } catch (e) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Status text - updated to reflect auto-conversion
          SizedBox(height: 6),
          Center(
            child: Text(
              'All images will be converted to WebP format. Max 10 MB',
              style: TextStyle(
                color: Colors.grey[600],
