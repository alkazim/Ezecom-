...
class WebHomeBannerWidget extends StatefulWidget {
  final double? width;
  final double? height;
  ...
  @override
  State<WebHomeBannerWidget> createState() => _WebHomeBannerWidgetState();
}

class _WebHomeBannerWidgetState extends State<WebHomeBannerWidget> {
  ...
  Future<void> _fetchImageUrls() async {
    try {
      ...
      final response = await Supabase.instance.client
          ...
          .eq('Active',
              true); // Changed to boolean true instead of string 'TRUE'
      ...
      for (var item in response) {
        if (item['Organization'] != null) {
          ...
          // Check if WebHome is true in the Organization JSON
          if (organization['WebHome'] == true) {
      ...
      print(
          'Banner Widget: Filtered to ${filteredRecords.length} records with WebHome=true');
      ...
      print('Banner Widget: Successfully loaded ${imageUrls.length} images');
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    if (imageUrls.isEmpty) {
      print('Banner Widget: Empty state - no images available');
      return Container(
        width: widget.width ?? double.infinity,
        height: widget.height ?? 200,
        child: Center(
    ...
    bool isMobile = MediaQuery.of(context).size.width < 768;
    print(
        'Banner Widget: Displaying ${imageUrls.length} images - isMobile: $isMobile');
