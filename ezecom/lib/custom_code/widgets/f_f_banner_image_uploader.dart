...
class FFBannerImageUploader extends StatefulWidget {
  // Renamed the class here
  const FFBannerImageUploader({
    Key? key,
    this.width = 85,
    this.height = 60,
  ...
  @override
  State<FFBannerImageUploader>
      createState() => // Updated createState to match new name
          _FFBannerImageUploaderState();
}

class _FFBannerImageUploaderState extends State<FFBannerImageUploader> {
  ...
  void initState() {
    debugPrint(
        'FFBannerImageUploader: initState called'); // Updated debug print
    super.initState();
  ...
  void _initializeImageState() {
    debugPrint(
        'FFBannerImageUploader: _initializeImageState called'); // Updated debug print
    try {
      ...
      debugPrint(
          'FFBannerImageUploader: Error in _initializeImageState: $e'); // Updated debug print
      setState(() {
        _hasImageFile = false;
        _errorMessage = null;
      });
  ...
  String? _getAppStateValue() {
    debugPrint(
        'FFBannerImageUploader: _getAppStateValue called for ${widget.appStateVariableName}'); // Updated debug print
    try {
  ...
  void _setAppStateValue(String value) {
    debugPrint(
        'FFBannerImageUploader: _setAppStateValue called for ${widget.appStateVariableName} with value: $value'); // Updated debug print
    try {
      final appState = FFAppState();
  ...
  String? _getDimensionsValue() {
    debugPrint(
        'FFBannerImageUploader: _getDimensionsValue called for ${widget.dimensionsVariableName}'); // Updated debug print
    try {
      final appState = FFAppState();
  ...
  void _setDimensionsValue(String value) {
    debugPrint(
        'FFBannerImageUploader: _setDimensionsValue called for ${widget.dimensionsVariableName} with value: $value'); // Updated debug print
    try {
      final appState = FFAppState();
  ...
  bool _isValidImageFile(Uint8List bytes) {
    debugPrint(
        'FFBannerImageUploader: _isValidImageFile called with bytes length: ${bytes.length}'); // Updated debug print
  ...
  Future<ui.Image> _getImageDimensions(Uint8List imageBytes) async {
    debugPrint(
        'FFBannerImageUploader: _getImageDimensions called'); // Updated debug print
  ...
  bool _validateImageResolution(int width, int height) {
    debugPrint(
        'FFBannerImageUploader: _validateImageResolution called with $width x $height'); // Updated debug print
  ...
  Future<Uint8List> _compressImage(Uint8List imageBytes) async {
    debugPrint(
        'FFBannerImageUploader: _compressImage called with bytes length: ${imageBytes.length}'); // Updated debug print
    try {
  ...
  Future<void> _pickImage() async {
    debugPrint(
        'FFBannerImageUploader: _pickImage called'); // Updated debug print
    setState(() {
      _isLoading = true;
      _errorMessage = null;
    });
  ...
  void _removeImage() {
    debugPrint(
        'FFBannerImageUploader: _removeImage called'); // Updated debug print
    try {
      setState(() {
        _fileName = '';
        _hasImageFile = false;
  ...
  void _showErrorSnackBar(String message) {
    debugPrint(
        'FFBannerImageUploader: Showing error snackbar: $message'); // Updated debug print
    if (mounted) {
  ...
  void _showSuccessSnackBar(String message) {
    debugPrint(
        'FFBannerImageUploader: Showing success snackbar: $message'); // Updated debug print
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(message),
  ...
  Widget _buildImageDisplay() {
    debugPrint(
        'FFBannerImageUploader: _buildImageDisplay called'); // Updated debug print
    return Container(
  ...
  Widget _buildAddButton() {
    debugPrint(
        'FFBannerImageUploader: _buildAddButton called'); // Updated debug print
  ...
  Widget build(BuildContext context) {
    debugPrint('FFBannerImageUploader: build called'); // Updated debug print
